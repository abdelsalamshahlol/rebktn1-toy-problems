...
Tree.prototype.map = function(callback) {
  ...
  // First traverse
  let newTree = arguments[1] || new Tree(callback(this.value));
  for(let i = 0 ; i < this.children.length; i++){
    newTree.addChild(callback(this.children[i]));
    if(this.children.length){
      this.children[i].map(callback, newTree)
    }
  }
  // pass each value to callback
  // generate a new tree with the results
  return newTree;
};
